 ...
 */
function xmldb_block_enhanced_sharing_cart_upgrade($oldversion = 0)
{
	global $DB;
	...
	if ($oldversion < 2011111100) {
		$table = new xmldb_table('enh_cart');
	...
	if ($oldversion < 2011111101) {
		$table = new xmldb_table('enh_cart_plugins');
	...
	if ($oldversion < 2012050800) {
		$table = new xmldb_table('enh_cart');
		$dbman->rename_table($table, 'block_enh_cart');
		
		$table = new xmldb_table('enh_cart_plugins');
		$dbman->rename_table($table, 'block_enh_cart_plugins');
	}

	if ($oldversion < 2016032900) {
		// Define key userid (foreign) to be added to block_enhanced_sharing_cart.
		$table = new xmldb_table('block_enh_cart');
		$key = new xmldb_key('userid', XMLDB_KEY_FOREIGN, array('userid'), 'user', array('id'));

		// Launch add key userid.
		$dbman->add_key($table, $key);

		// enhanced_Sharing_cart savepoint reached.
		upgrade_block_savepoint(true, 2016032900, 'enhanced_sharing_cart');
	}
	...
	if($oldversion < 2017071111)
    {
        ...
        upgrade_block_savepoint(true, 2017071111, 'enhanced_sharing_cart');
    }
    ...
    if($oldversion < 2017121200)
    {
        ...
        $table = new xmldb_table('block_enh_cart_sections');
        ...
        upgrade_block_savepoint(true, 2017121200, 'enhanced_sharing_cart');
    }
