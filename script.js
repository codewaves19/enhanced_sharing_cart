...
require(['jquery'], function ($)
{
    $(document).ready(function()
    {
        ...
        /** @var {Node}  The enhanced Sharing Cart block container node */
        var $block = $('.block_enhanced_sharing_cart');

        var $spinner_modal = {
            show: function()
            {
                $('#enhanced_sharing-cart-spinner-modal').show();
            },
            hide: function()
            {
                $('#enhanced_sharing-cart-spinner-modal').hide();
            }
        };
        ...
        {
            return M.str.block_enhanced_sharing_cart[identifier] || M.str.moodle[identifier];
        }
        ...
        /**
         *  Reload the Enhanced Sharing Cart item tree
         */
        ...
        {
            var KEY = 'block_enhanced_sharing_cart-dirs';
            ...
            this.init = function ()
            {
                ...
                {
                    var $dir = $(dir);
                    $dir.attr('id', 'block_enhanced_sharing_cart-dir-' + i);
                    if (i >= opens.length)
                    {
                        opens.push(0);
                    }
        ...
        var move_targets = new function ()
        {
            ...
            this.show = function (id)
            {
                ...
                var $current = $block.find('#block_enhanced_sharing_cart-item-' + id);
        ...
        var restore_targets = new function ()
        {
            ...
            this.show = function (id)
            {
                ...
                {
                    var $item = $block.find('#block_enhanced_sharing_cart-item-' + id);
        ...
        $.init_help_icon = function (isspeciallayout)
        {
            ...
            if (isspeciallayout)
            {
                helpicon.attr('data-placement', 'left').find('.help-icon')
                    .prepend($('<span/>').append(M.str.block_enhanced_sharing_cart['pluginname']).addClass('sc-space-5'));
        ...
        /**
         *  Initialize the Enhanced Sharing Cart block header
         */
        ...
        /**
         *  Initialize the Enhanced Sharing Cart item tree for BLOCK
         */
        ...
        $.init_activity_commands = function () // iniiatise all activity commands  for BLOCK
        {
            function add_backup_comand($activity) // function to add backup command on every activity  for BLOCK
            {
                var $menu = $activity.find("ul[role='menu']"); // check this@@@
                ...
                {
                    //Create a command icon for moodle 3.2  for BLOCK
            ...
            if(course.is_frontpage) // if on frontpage of course area
            {
                $(".sitetopic li.activity").each(function()
                {
                    add_backup_comand($(this));  // add backup command to each activity with sitetopic class
                });
            ...
            {
                $(".course-content li.activity").each(function()       // At course-content area where li.activity
                {
                    add_backup_comand($(this));
                });
            }

            $("li.section").each(function()  // li with class=section ie all the sections
            {
                var sectionID = $(this).find("div.content h3.sectionname span.inplaceeditable").attr("data-itemid");  // section name in single section whichis editable

                var $menu = $(this).find("ul[role='menu']").first(); //activity dropdown button menu has ul[role='menu]
                ...
                if($menu.length)
                {
                    ...
                    if (img.length) {
                        img.attr('alt', str('backup')).attr('title', str('backup')).attr('src', M.util.image_url(icon['backup'].pix, null)); // image alt back string title backup string source with image url 
                    } else {
                        li.find('i').attr('class', 'icon fa fa-upload').attr('title', str('backup')).attr('aria-label', str('backup')); // icon title backup string aria-label attribute backup string
                    }

                    li.find('span').html(str('backup')); // li span backup string is find
                    li.find('a').attr('href', 'javascript:void(0)'); // href attribute is find
                ...
                { // for section edit menu dropdown
                    ...
                    else
                    {
                        ...
                        $activity.find(".commands").append($backup); // in block activities with .command class
                    }

                    $backup.click(function () // menu item copy to enhanced sharing cart is clicked
                    {
                        $.on_section_backup(sectionID); 
        ...
        /**
         * Initialize the Enhanced Sharing Cart Block
         */
        $.init = function()
        {
            M.str.block_enhanced_sharing_cart['pluginname'] = this.get_plugin_name();
        ...
        $('div#enhanced_sharing-cart-spinner-modal div.spinner-container').prepend($spinner);
