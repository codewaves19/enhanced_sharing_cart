 ...
 */
YUI.add('block_enhanced_sharing_cart', function (Y)
{
    ...
    M.block_enhanced_sharing_cart = new function ()
    {
        ...
        var $block = Y.Node.one('.block_enhanced_sharing_cart');
        ...
        {
            return M.str.block_enhanced_sharing_cart[identifier]
            ...
            Y.io(get_action_url('rest'), {
                ...
                on: {
                    ...
                    success: function (tid, response)
                    {
                        ...
                        M.block_enhanced_sharing_cart.init_item_tree();
                    },
        ...
        /**
         *  Backup an activity  // no effect on activity backup, create some node above page content not visible
         *  
         *  @param {Integer} cmid
        ...
        var move_targets = new function ()
        {
            ...
            /** if cancel the hide the targets
             *  Hide move targets
             */
            this.hide = function ()
            {
            ...
            this.show = function (id)
            {
                ...
                var $current = $block.one('#block_enhanced_sharing_cart-item-' + id); //activity DOM structure
                var $indent = $current.one('div');
                var $next = $current.next();
                var $list = $current.ancestor('ul');
                
                var next_id = $next ? $next.get('id').match(/item-(\d+)$/)[1] : 0; //activity DOM structure with new ID incremented by one
                
                /* ##### Function to create targets on the course page #### manisha */
                function create_target(id, to) 
                {
                    var $anchor = Y.Node.create('<a href="javascript:void(0)"/>') // create a new node for the targets
                    ...
                    var $target = Y.Node.create('<li class="section main"/>')
                        .append($indent.cloneNode(false).append($anchor));
                    $anchor.on('click', move, this);
                    return $target;
                } // End function create_target
        ...
        var restore_targets = new function ()
        {
            ...
            {
                this.hide();
                var $item = $block.one('#block_enhanced_sharing_cart-item-' + id);
                
                $clipboard = Y.Node.create('<div class="clipboard"/>'); // display this on course page top
        ...
        var directories = new function ()
        {
            ...
            this.init = function ()
            {
                ...
                {
                    $dir.set('id', 'block_enhanced_sharing_cart-dir-' + i);
                    if (i >= opens.length)
                        opens.push(0);
                    else if (opens[i])
                        open($dir, true);
        ...
        this.init_help_icon = function (isspeciallayout)
        {
            ...
            if (isspeciallayout) {
                ...
                helpicon = helpicon.prepend(Y.Node.create('<span/>').append(M.str.block_enhanced_sharing_cart['pluginname']));
        ...
        /**
         *  On backup command clicked in activities
         *  
        ...
        this.on_backup = function (e)
        {
            ...
            })(function (response)
            {
                ...
                if (copyable) {
                    var $yesnocancel = new M.block_enhanced_sharing_cart.yesnocancel({
                        title: str('backup'),
    ...
    M.block_enhanced_sharing_cart.yesnocancel = YESNOCANCEL;
},
