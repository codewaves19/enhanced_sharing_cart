...
if (is_array($delete_param)) try {
	...
	$records = $DB->get_records_select(enhanced_sharing_cart\record::TABLE, "userid = $USER->id AND id $sql", $params);
	if (!$records)
		throw new enhanced_sharing_cart\exception('recordnotfound');
	
	$storage = new enhanced_sharing_cart\storage();
	...
	$DB->delete_records_select(enhanced_sharing_cart\record::TABLE, "id $sql", $params);
	
	enhanced_sharing_cart\record::renumber($USER->id);
...
$items = $DB->get_records(enhanced_sharing_cart\record::TABLE, array('userid' => $USER->id), $orderby);

$title = get_string('bulkdelete', 'block_enhanced_sharing_cart');

$PAGE->set_pagelayout('standard');
$PAGE->set_url('/blocks/enhanced_sharing_cart/bulkdelete.php', array('course' => $courseid));
$PAGE->set_title($title);
$PAGE->set_heading($title);
$PAGE->navbar->add(get_string('pluginname', 'block_enhanced_sharing_cart'))->add($title, '');
...
echo $OUTPUT->header();
{
	...
	if (empty($items)) {
		...
		//<![CDATA[
			...
			{
				return confirm("', s(
					get_string('confirm_delete_selected', 'block_enhanced_sharing_cart')
				), '");
			}
			function check()
			{
		...
		foreach ($items as $id => $item) {
			...
			<li style="list-style-type:none; clear:left;">
				...
				<div style="float:left;">', enhanced_sharing_cart\renderer::render_modicon($item), '</div>
